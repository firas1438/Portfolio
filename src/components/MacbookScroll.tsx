...
export const MacbookScroll = ({ src, badge, }: { src?: string; title?: string | React.ReactNode; badge?: React.ReactNode; }) => {
  ...
  const translate = useTransform(scrollYProgress, [0, 1], [5, 100]);
  const rotate = useTransform(scrollYProgress, [0.1, 0.12, 0.3], [-28, -28, 0]);
 
  return (
    <div ref={ref}
      className=" flex shrink-0 flex-col items-center justify-start py-0 lg:py-10 perspective-midrange
        scale-[0.39] sm:scale-[0.7] md:scale-[0.8] lg:scale-100 -mt-28 sm:mt-0 transition-all duration-300 "
    >
      {/* Lid */}
